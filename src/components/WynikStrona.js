...
function WynikStrona() {
  ...
  const getImageForPercentage = (percentage) => {
    if (percentage > 35) return image0;
    if (percentage > 20) return image1;
    if (percentage > 5) return image2;
    if (percentage > 1) return image3;
    return image4;
  };

  // Placeholder descriptions based on percentage range
  const getDescriptionForPercentage = (percentage) => {
    if (percentage > 35) return '✨ Naprawdę nie prosisz o wiele! Każda szanująca się kobieta musi oczekiwać od partnera zdrowego minimum. 😌 Skoro Twój wybranek czeka za rogiem, życzymy powodzenia! 🎉 Nie zapomnij o darowiźnie dla lokalnego schroniska 🐾, bo Twój przyszły niedoszły koci towarzysz prędko się z Tobą nie spotka! 😹';

    if (percentage > 20) return '🔮 Na dwoje babka wróżyła! Chłopcy w Twoim typie istnieją, ale nie leżą w każdym mięsnym. 😅 Jak swojego amanta nie znajdziesz, zawsze możesz na randkę iść z kotem 🐱. Jeśli planujesz szukać partnera na poważnie, nie poddawaj się, ale darowizna na schronisko też nie zaszkodzi. Może Twój chłopak tam mieszka! 🏠😹';
    
    if (percentage > 5) return '😬 Ciężki przypadek, laleczko! Typów w Twoim typie jest mniej niż się wydaje 🥲 i zdecydowanie mniej niż kocurów w okolicy. Możesz liczyć na uwagę kociego przyjaciela, ale czy o takie życie Ci chodzi? 🍷 Propozycja randki? Butelka wina i puszka mokrej karmy! 😽 Darowiznę wpłacisz o tutaj 🐾.';
    
    if (percentage > 1) return '😿 Miaaau! Nie jest dobrze. Typów na horyzoncie brak. Koty w Twoim towarzystwie już nawet nie interesują się saszetą 🐟, skazane na Ciebie, patrzą smutno na butelkę wina 🍷 – produkt pierwszej potrzeby. Chlip. Przemyśl życiowe decyzje, a może odwiedź lokalne schronisko 🐈 w międzyczasie.';
